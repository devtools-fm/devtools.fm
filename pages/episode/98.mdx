...
youtube: https://www.youtube.com/watch?v=BfXgV5s9Ki4
tags: technology, nodejs, deno, javascript, programming, web development
---

<!-- TAB: SHOW NOTES -->

This week we have Ryan Dahl, the creator of Node.js and Deno. We talk about the history of Node.js, the creation of Deno, and the future of JavaScript.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** web browser. It's it is for interacting with file systems and TCP sockets and all sorts of stuff that you can't do in web browsers.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** And, 
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** Good, thanks. Yeah, I, I think I agree with you. Yeah, you know, when, uh, you know, in the 1st year of node, there wasn't even a way to distribute packages, right? This, this was largely invented by Isaac Schlueter, uh, and we built this, this NPM system, which, uh, Yeah, I mean, it's it's it's kind of hard to think back to 15 years ago, but I mean, there was a time when like, none of this was worked out at all.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** gets set into stone and, you know, maybe you're porting stuff into rust, you're kind of defining the structures and yeah, I think at this point in 2024.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** which is a web framework. Um, it's 

**Ryan:** not part of Deno itself. So, 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** you know, it's, uh, I think, uh, You know, right for right for this, this area, but you know, I, I wouldn't want to make the claim that fresh is the way that you develop websites. I, I think there is still rapid iteration happening in the web framework space.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** most, uh, it's kind of hard to quantify actually, but most of the note built in node API is implemented. So the like node, node colon FS, like you can just import that and use that.

And, and that's, that's, uh, deeply 

**Ryan:** baked into Deno. You can 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** also just import, uh. NPM packages and and because most NPM packages are are written in common JS with with require although you can't use require as a 

**Ryan:** top level Deno user we we 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** kind of prevent you from from doing that uh we you can't actually 

**Ryan:** you know Deno does have 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** an implementation of common JS because it needs to support all of these these NPM modules uh this has been a really long journey because the the long tail of uh Uh, compatibility is, is hard.

Um, but we, we continue to, to hammer away 

**Ryan:** at it and Deno is getting 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** better and better at it every day. Um, you know, sometimes it seems like it's going to go on forever, but I mean, these days, you know, can run next JS, it can run most web frameworks and we're working 

**Ryan:** towards a, uh, Deno to release 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** where we make this really explicit and are just like, You know, you can you can 

**Ryan:** essentially use Deno in all of 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** your existing node projects.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** matter if it's using private registries or if it's using NPM workspaces. It all just 

**Ryan:** works in Deno.

Um, and, 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** uh, yeah, we're, we're, we're, we're pretty close to that. That's, that's just a couple of months away.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** Yeah, J. J. S. R. stands for javascript registry. Uh, and it is a new registry kind of like like N. P. M. Uh, and you know, it's important to differentiate this from from yarn or P. N. P. M. which are package managers that still talk to the same N. P. M. registry. J. S. R. is really a new storage location, a new distribution marketplace for, for, uh, for javascript and typescript code.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** we, we built it, um, it, it, it is open source and, and it's intends to be, uh, openly managed, um, you know, surprisingly like the NPM registry isn't, isn't even open source. So, you know, we, we are really intending this as a public service and, uh, essentially want to, want to put it into the public domain once, once we kind of find a, uh, Uh, staff of, uh, you know, some, some people that, that, uh, that are sufficiently, uh, motivated to, to kind of, uh, moderate and keep it, it nice.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** or, uh, S. M. dot S. H. Uh, the reliability of various U. R. L. S. was, was pretty bad. And so you, you start having this. Ecosystem of, of modules that link to random websites.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** treats them at abstractly as as two different modules rather than the same module.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** will continue to support HEP 

**Ryan:** modules and Deno Land slash 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** X will continue to be there indefinitely. But JSR is really kind of the next 

**Ryan:** iteration of Deno Land slash 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** X.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** X. And ultimately after thinking, you know, just dwelling on this for forever, just, uh, decided that, that we really needed to create JSR that, that, uh, that JavaScript demanded it.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** Oh yeah, yeah, it, it, it totally is. Um, it's just, uh, Yeah, it's, it's things that you would expect, right? Uh, so it ends like, like a lot of standards work, which I, you know, uh, applaud the people who participate in, in these things, but I myself find incredibly boring. Uh, it's, it's, uh, you know, it's, it's, uh, uh, It's good to do these, these sort of things, but you know, it is, it is really just, just kind of, uh, shading in the Venn diagram of browser and server side stuff and, and just, just kind of clarifying, uh, in a lot of technical areas, what, what, what the request object is doing and how that streams and, and that sort of stuff um you know, I think the big thing 
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** Yeah. I, I, um, There's a through line through all, all of my work, uh, from, from nodes to, to 

**Ryan:** Deno, to Deno deploy to, 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** to JSR, which is, I like scripting languages. I, you know, my, I, myself obviously use scripting language is, uh, but you know, I, I'm more of a C plus plus programmer, more, more of a, more of a rust programmer, actually.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** to do. That's what J. S. R. Is trying to do where you just you just publish the typescript that you write, and 

**Ryan:** that's what Deno deploy is 

**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** is trying to do too. Uh, but in in kind of the cloud hosting sense, so you really just focus on your business logic, your three lines of JavaScript.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** app. And then, you know, on, on every commit, you can create a new subdomain for you and, and it's kind of, uh, uh, run your, your code in a serverless fashion.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** through a JavaScript API can like literally store JavaScript objects and behind the scenes does a V8 serialization.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** and give a call back.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** is simply how many tenants 
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** that's that's a good idea.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** I really believe in open source. So node was MIT licensed. Deno is MIT licensed. You know, that is like very, very free. It's it's like, you can take this software and fork it and build commercial stuff on it. And I will have nothing to say about that. I think that is inspiring. You know, in some ways like super crazy, uh, because like software is, is very expensive to develop.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** my philosophy is, is kind of. Let's, let's create as much, much stuff in the open as, as possible. And, uh, you know, people, people will use that and that that's going to afford opportunities to provide them with commercial services.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** is, is, is proprietary. So not everything is, is open source, but you know, we, we really do try to try to participate and code in the open.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** that that we will continue to see a future where the cloud is dominated by JavaScript for business logic.
...
**riverside_ryan_dahl_raw-synced-video-cfr_devtools:** Absolutely. Thank you for having me.
